...
.PHONY: tag-app
tag-app:
	docker tag $(APP_IMAGE_NAME):latest $(ECR_REGISTRY)/$(ELASTIC_SERVICE_REGISTRY_NAME):latest
...
.PHONY: push-app
push-app: tag-app
	docker push $(ECR_REGISTRY)/$(ELASTIC_SERVICE_REGISTRY_NAME):latest
...
clean-registry-image:
	docker rmi -f $(ECR_REGISTRY)/$(ELASTIC_SERVICE_REGISTRY_NAME):latest
