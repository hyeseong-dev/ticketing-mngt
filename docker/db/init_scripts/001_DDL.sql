...
CREATE TABLE refresh_token (
                               ...
                               device_info VARCHAR(30) NULL COMMENT '디바이스 정보(예: User-Agent Info on HTTP REQUEST)',
                               ...
                               CONSTRAINT fk_refresh_token_user FOREIGN KEY (user_id) REFERENCES users (user_id) -- 수정: id -> user_id
) COMMENT '사용자의 리프레시 토큰을 저장하는 테이블';

-- 장소 테이블
CREATE TABLE place (
                       place_id BIGINT NOT NULL AUTO_INCREMENT COMMENT '공연장 ID (기본 키)',
                       name VARCHAR(30) NOT NULL COMMENT '공연장 이름',
                       seats_cnt INT NOT NULL COMMENT '좌석 개수',
...
CREATE TABLE seat (
                      ...
                      status ENUM('SEAT_AVAILABLE', 'SEAT_RESERVED') NOT NULL COMMENT '예약 상태',
                      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '생성 일시',
                      updated_at DATETIME DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP COMMENT '수정 일시',
                      PRIMARY KEY (seat_id),
...
CREATE TABLE reservation (
                             ...
                             status ENUM('PENDING', 'CONFIRMED', 'CANCELLED') NOT NULL COMMENT '예약 상태',
...
CREATE TABLE payment (
                         ...
                         payment_price DECIMAL(10, 2) NOT NULL COMMENT '결제 금액',
                         status ENUM('PAYMENT_READY', 'PAID', 'CANCELLED') NOT NULL COMMENT '결제 상태',
